                    ...
                    {isLoggedIn ? (
                        <div className="relative group">
                            ...
                            <div className="absolute hidden group-hover:block top-6 right-0 pt-4 ">
                                <button onClick={()=>logout()} className="bg-white/20 border-2 border-white/10 px-5 py-1.5 rounded ">Logout
                                </button>
                            </div>
                        </div>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <Link onClick={() => setIsOpen(false)} to='/generate'>Generate</Link>
                {
                    isLoggedIn ?  <Link onClick={() => setIsOpen(false)} to='/my-generation' >My Generations</Link> : 
                     <Link onClick={() => setIsOpen(false)} to='/#contact' >About</Link>                 
                }
                
                <Link onClick={() => setIsOpen(false)} to='#/contact'>Contact us</Link>
                {
                    isLoggedIn ? <button onClick={()=>{setIsOpen(false);logout()}}>Logout</button> :
                    <button onClick={()=>{navigate('/login');setIsOpen(false)}} className="px-6 py-2.5 bg-pink-600 hover:bg-pink-700 active:scale-95 transition-all rounded-full">Get Started</button>
                }
